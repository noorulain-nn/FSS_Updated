...
def load_backbone_seg(backbone_name: str):
    """
    Same freeze strategy as load_backbone() — only last block trains.
    Difference: NO GlobalAvgPool and NO Flatten at the end.
    Returns spatial feature map [B, C, H', W'] instead of [B, feat_dim].

    This is required for segmentation because we need per-pixel features.

    Returns:
        encoder  : nn.Module   call as feats = encoder(imgs)
        feat_dim : int         number of channels in output feature map
    """
    name = backbone_name.lower().strip()
    ...
    if name in {"resnet18", "resnet34", "resnet50", "resnet101"}:
        ...
        # Freeze everything except layer4 — same as classification APM
        for n, p in m.named_parameters():
            if not n.startswith("layer4."):
                p.requires_grad = False
        ...
        # Output: [B, feat_dim, H/32, W/32]
        # No avgpool, no flatten — spatial map preserved
    ...
    if name in {"densenet121", "densenet161", "densenet169"}:
        ...
        feat_dim = m.classifier.in_features  # 1024 / 2208 / 1664

        # Output: [B, feat_dim, H/32, W/32]
        encoder = nn.Sequential(
            m.features,
            nn.ReLU(inplace=True)
            # No AdaptiveAvgPool2d, no Flatten
        )
        return encoder, feat_dim
    ...
    raise ValueError(f"Unsupported backbone for segmentation: {backbone_name!r}")