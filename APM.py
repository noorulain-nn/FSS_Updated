...
class SegAPM(nn.Module):
    """
    Full APM-FSS model: backbone encoder + FSS memory + segmentation head.

    The methodology exactly mirrors APM classification:
      - Same image used for both loss computation and memory update
      - Only last backbone block fine-tuned
      - Memory head is non-parametric (same EMA update, no gradients)
      - Segmentation head is a small 2-layer conv (only new addition)
    """
    ...
    def __init__(self,
        ...
        # Input : similarity map [B, num_classes, H', W']
        # Output: binary logits  [B, 2, H, W]  (2 = background + foreground)
    ...
    def forward(self, imgs: torch.Tensor):
        ...
        Args:
            imgs : [B, 3, H, W]
        Returns:
            seg_logits     : [B, 2, H, W]    for loss/prediction
            features       : [B, C, H', W']  for memory update
            similarity_map : [B, K, H', W']  for inspection
        """
        features       = self.encode(imgs)                   # [B, C, H', W']
        similarity_map = self.memory_module(features)         # [B, K, H', W']
        seg_out        = self.seg_head(similarity_map)        # [B, 2, H', W']
        ...
        return seg_logits, features, similarity_map
